...
class CustomPlotMatplot(BasePlot):
    ...
    def plot_obs(self, observation: CompleteObservation,
        ...
        if self.old_topo is not None:
            non_equal_indices = np.where((self.old_topo == observation.topo_vect) == 0)
        self.old_topo = observation.topo_vect

        if not figure:
            figure = plt.gcf()

        # Plot rho
        ax1 = figure.add_subplot(self.gs[0, 2])
        ax1.set_title('Maximal overflow in any line')
        ax1.set_ylim(0.5, 2.0)
        if self.step_counter <= self.history:
            plt.axline((0, 1), (self.history, 1), color='red')
            ...
            plt.axline((self.step_counter - len(self.max_rho[-self.history::]), 1), (self.step_counter - 1, 1),
                       color='red')
            ax1.plot(np.arange(self.step_counter - len(self.max_rho[-self.history::]), self.step_counter),
                     self.max_rho[-self.history::])

        # Plot active lines
        ax2 = figure.add_subplot(self.gs[1, 2])
        ax2.set_title('Number of active lines')
        ...
        ax3 = figure.add_subplot(self.gs[2, 2])
        ax3.set_title('Number of overflown lines')
    ...
    def create_figure(self):
        ...
        self.gs = GridSpec(3, 3, figure=f)
        self.ax = f.add_subplot(self.gs[:, :2])
        f.canvas.draw()
        return f
